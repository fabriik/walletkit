...
#include "BRWOCParams.h"

static const char *BRWOCMainNetDNSSeeds[] = {
...
static const char *BRWOCTestNetDNSSeeds[] = {
...
static const BRCheckPoint BRWOCMainNetCheckpoints[] = {
...
static const BRCheckPoint BRWOCTestNetCheckpoints[] = {
    ...
    { 1500000, uint256("000000000000317a7b535ead7a8f38ef50ee47d3dbf8f5e8597085bf22049dc5"), 1612165617, 0x1c00d3c7 }
    // 2016000
};

static int BRWOCMainNetVerifyDifficulty(const BRMerkleBlock *block, const BRSet *blockSet)
{
...
static int BRWOCTestNetVerifyDifficulty(const BRMerkleBlock *block, const BRSet *blockSet)
{
    return 1; // XXX skip testnet difficulty check for now
}

extern const BRCheckPoint *BRWOCChainParamsGetCheckpointBefore (const BRChainParams *params, uint32_t timestamp) {
...
extern const BRCheckPoint *BRWOCChainParamsGetCheckpointBeforeBlockNumber (const BRChainParams *params, uint32_t blockNumber) {
    for (ssize_t index = (ssize_t) (params->checkpointsCount - 1); index >= 0; index--)
...
static const BRChainParams BRWOCParamsRecord = {
    BRWOCMainNetDNSSeeds,
    8333,                  // standardPort
    0xd9b4bef9,            // magicNumber
    SERVICES_NODE_WITNESS, // services
    BRWOCMainNetVerifyDifficulty,
    BRWOCMainNetCheckpoints,
    sizeof(BRWOCMainNetCheckpoints)/sizeof(*BRWOCMainNetCheckpoints),
    { BITCOIN_PUBKEY_PREFIX, BITCOIN_SCRIPT_PREFIX, BITCOIN_PRIVKEY_PREFIX, BITCOIN_BECH32_PREFIX },
    BITCOIN_FORKID
};
const BRChainParams *BRWOCParams = &BRWOCParamsRecord;

static const BRChainParams BRWOCTestNetParamsRecord = {
    BRWOCTestNetDNSSeeds,
    18333,                 // standardPort
    0x0709110b,            // magicNumber
    SERVICES_NODE_WITNESS, // services
    BRWOCTestNetVerifyDifficulty,
    BRWOCTestNetCheckpoints,
    sizeof(BRWOCTestNetCheckpoints)/sizeof(*BRWOCTestNetCheckpoints),
    { BITCOIN_PUBKEY_PREFIX_TEST, BITCOIN_SCRIPT_PREFIX_TEST, BITCOIN_PRIVKEY_PREFIX_TEST, BITCOIN_BECH32_PREFIX_TEST },
    BITCOIN_FORKID
};

const BRChainParams *BRWOCTestNetParams = &BRWOCTestNetParamsRecord;
