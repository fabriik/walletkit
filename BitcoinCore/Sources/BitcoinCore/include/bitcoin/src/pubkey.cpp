...
int ecdsa_signature_parse_der_lax(const secp256k1_context* ctx, secp256k1_ecdsa_signature* sig, const unsigned char *input, size_t inputlen) {
    ...
    secp256k1_bitcoin_ecdsa_signature_parse_compact(ctx, sig, tmpsig);
    ...
    if (!overflow) {
        overflow = !secp256k1_bitcoin_ecdsa_signature_parse_compact(ctx, sig, tmpsig);
    }
    if (overflow) {
...
bool CPubKey::Verify(const uint256 &hash, const std::vector<unsigned char>& vchSig) const {
    ...
    if (!secp256k1_bitcoin_ec_pubkey_parse(secp256k1_context_verify, &pubkey, vch, size())) {
        return false;
    }
    ...
    secp256k1_bitcoin_ecdsa_signature_normalize(secp256k1_context_verify, &sig, &sig);
    return secp256k1_bitcoin_ecdsa_verify(secp256k1_context_verify, &sig, hash.begin(), &pubkey);
}
...
bool CPubKey::RecoverCompact(const uint256 &hash, const std::vector<unsigned char>& vchSig) {
    ...
    size_t publen = SIZE;
    secp256k1_bitcoin_ec_pubkey_serialize(secp256k1_context_verify, pub, &publen, &pubkey, fComp ? SECP256K1_EC_COMPRESSED : SECP256K1_EC_UNCOMPRESSED);
    Set(pub, pub + publen);
    return true;
}
...
bool CPubKey::IsFullyValid() const {
    ...
    secp256k1_pubkey pubkey;
    assert(secp256k1_context_verify && "secp256k1_context_verify must be initialized to use CPubKey.");
    return secp256k1_bitcoin_ec_pubkey_parse(secp256k1_context_verify, &pubkey, vch, size());
}
...
bool CPubKey::Decompress() {
    ...
    unsigned char pub[SIZE];
    size_t publen = SIZE;
    secp256k1_bitcoin_ec_pubkey_serialize(secp256k1_context_verify, pub, &publen, &pubkey, SECP256K1_EC_UNCOMPRESSED);
    Set(pub, pub + publen);
    return true;
}
...
bool CPubKey::Derive(CPubKey& pubkeyChild, ChainCode &ccChild, unsigned int nChild, const ChainCode& cc) const {
    ...
    if (!secp256k1_bitcoin_ec_pubkey_tweak_add(secp256k1_context_verify, &pubkey, out)) {
        return false;
    }
    unsigned char pub[COMPRESSED_SIZE];
    size_t publen = COMPRESSED_SIZE;
    secp256k1_bitcoin_ec_pubkey_serialize(secp256k1_context_verify, pub, &publen, &pubkey, SECP256K1_EC_COMPRESSED);
...
/* static */ bool CPubKey::CheckLowS(const std::vector<unsigned char>& vchSig) {
    ...
    return (!secp256k1_bitcoin_ecdsa_signature_normalize(secp256k1_context_verify, nullptr, &sig));
}
...
ECCVerifyHandle::ECCVerifyHandle()
{
    if (refcount == 0) {
        ...
        secp256k1_context_verify = secp256k1_bitcoin_context_create(SECP256K1_CONTEXT_VERIFY);
        assert(secp256k1_context_verify != nullptr);
    }
    refcount++;
}
...
ECCVerifyHandle::~ECCVerifyHandle()
{
    ...
    if (refcount == 0) {
        assert(secp256k1_context_verify != nullptr);
        secp256k1_bitcoin_context_destroy(secp256k1_context_verify);
