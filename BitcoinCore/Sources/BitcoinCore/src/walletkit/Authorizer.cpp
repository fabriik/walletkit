...
static std::vector<unsigned char> getLittleEndian(CKey privKey, std::vector<unsigned char> sig, std::vector<unsigned char> hashBuf) {
    ...
    std::string d = hex2dec.Convert(bnStr.c_str());

    std::string r = hex2dec.Convert(rStr.c_str());

    std::string e = hex2dec.Convert(hashBufStr.c_str());
...
static TokenDataStruct getTokenDataFromOutput (PaymentOutput output, std::string address) {
    ...
    char ch[16+1];
    sprintf(ch, "%016x", (unsigned int) tokenData.amount);
    std::string str(ch);
    std::string hex_buf = uchbufToString(toLittleEndianUch(hexToUchBuffer(str)));
    tokenData.state = hex_buf + std::string("080000");
...
class SigOperations {
    ...
    public:
        ...
        std::string toJSON() {
            ...
            std::string sigOpCountStr("\"sigOpCount\": ");
            std::string nScriptChunkStr("\"nScriptChunk\": ");
            std::string typeStr("\"type\": ");
            std::string addressStr("\"addressStr\": ");
            std::string nHashTypeStr("\"nHashType\": ");
            ...
            for (it = map.begin(); it != map.end(); it++)
            {
                ...
                if(it == map.begin()) {
                    labelStr = labelStr + std::string("\"") + it->first + std::string("\"");
                    sigOpCountStr = sigOpCountStr + std::string("\"") + std::to_string(arr.size()) + std::string("\"");
                } else {
                    labelStr = labelStr + std::string(", \"") + it->first + std::string("\"");
                    sigOpCountStr = sigOpCountStr + std::string(", \"") + std::to_string(arr.size()) + std::string("\"");
                }
                for(size_t i = 0; i < arr.size(); i++) {
                    if(it == map.begin() && i == 0) {
                        nScriptChunkStr = nScriptChunkStr + std::string("\"") + std::to_string(arr[i].nScriptChunk) + std::string("\"");
                        typeStr = typeStr + std::string("\"") + arr[i].type + std::string("\"");
                        addressStr = addressStr + std::string("\"") + arr[i].addressStr + std::string("\"");
                        nHashTypeStr = nHashTypeStr + std::string("\"") + std::to_string(arr[i].nHashType) + std::string("\"");
                    } else {
                        nScriptChunkStr = nScriptChunkStr + std::string(", \"") + std::to_string(arr[i].nScriptChunk) + std::string("\"");
                        typeStr = typeStr + std::string(", \"") + arr[i].type + std::string("\"");
                        addressStr = addressStr + std::string(", \"") + arr[i].addressStr + std::string("\"");
            ...
            labelStr = labelStr + std::string(",");
            sigOpCountStr = sigOpCountStr + std::string(",");
            nScriptChunkStr = nScriptChunkStr + std::string(",");
            typeStr = typeStr + std::string(",");
            addressStr = addressStr + std::string(",");
            nHashTypeStr = nHashTypeStr + std::string(" }");
            
            return labelStr + sigOpCountStr + nScriptChunkStr + typeStr + addressStr + nHashTypeStr;
        }
    
        void fromJSON(std::string json) {
            SigOperations sigOps;
            std::string delimiterLabel("label");
            std::string delimiterSigOpCount("sigOpCount");
            std::string delimiterNScriptChunk("nScriptChunk");
            std::string delimiterType("type");
            std::string delimiterAddressStr("addressStr");
            std::string delimiterNHashType("nHashType");
            std::string delimiterComma("\", \"");
            std::string delimiterCloseBrace("}");
            
            std::vector<std::string> labelVec;
            std::vector<std::string> sigOpCountVec;
            std::vector<std::string> nScriptChunkVec;
            std::vector<std::string> typeVec;
            std::vector<std::string> addressStrVec;
            std::vector<std::string> nHashTypeVec;
            
            std::string labels = json.substr(json.find(delimiterLabel) + 8, (json.find(delimiterSigOpCount) - (json.find(delimiterLabel) + 8 + 2)));
            printf("labels = %s\n", labels.c_str());
            
            std::string sigOpCounts = json.substr(json.find(delimiterSigOpCount) + 13, (json.find(delimiterNScriptChunk) - (json.find(delimiterSigOpCount) + 13 + 2)));
            printf("sigOpCounts = %s\n", sigOpCounts.c_str());
            
            std::string nScriptChunks = json.substr(json.find(delimiterNScriptChunk) + 15, (json.find(delimiterType) - (json.find(delimiterNScriptChunk) + 15 + 2)));
            printf("nScriptChunks = %s\n", nScriptChunks.c_str());
            
            std::string types = json.substr(json.find(delimiterType) + 7, (json.find(delimiterAddressStr) - (json.find(delimiterType) + 7 + 2)));
            printf("types = %s\n", types.c_str());
            
            std::string addressStrs = json.substr(json.find(delimiterAddressStr) + 13, (json.find(delimiterNHashType) - (json.find(delimiterAddressStr) + 13 + 2)));
            printf("addressStrs = %s\n", addressStrs.c_str());
            
            std::string nHashTypes = json.substr(json.find(delimiterNHashType) + 12, (json.find(delimiterCloseBrace) - (json.find(delimiterNHashType) + 12 + 1)));
            printf("nHashTypes = %s\n", nHashTypes.c_str());
            
            size_t pos = 0;
            std::string token;
            labels = labels.substr(1, labels.length()-2);
            while ((pos = labels.find(delimiterComma)) != std::string::npos) {
                token = labels.substr(0, pos);
                //printf("token = %s\n", token.c_str());
                labelVec.push_back(token);
                labels.erase(0, pos + delimiterComma.length());
            }
            labelVec.push_back(labels);
            //printf("labels = %s\n", labels.c_str());
            
            pos = 0;
            sigOpCounts = sigOpCounts.substr(1, sigOpCounts.length()-2);
            while ((pos = sigOpCounts.find(delimiterComma)) != std::string::npos) {
                token = sigOpCounts.substr(0, pos);
                sigOpCountVec.push_back(token);
                sigOpCounts.erase(0, pos + delimiterComma.length());
            }
            sigOpCountVec.push_back(sigOpCounts);
            
            pos = 0;
            nScriptChunks = nScriptChunks.substr(1, nScriptChunks.length()-2);
            while ((pos = nScriptChunks.find(delimiterComma)) != std::string::npos) {
                token = nScriptChunks.substr(0, pos);
                nScriptChunkVec.push_back(token);
                nScriptChunks.erase(0, pos + delimiterComma.length());
            }
            nScriptChunkVec.push_back(nScriptChunks);
            
            pos = 0;
            types = types.substr(1, types.length()-2);
            while ((pos = types.find(delimiterComma)) != std::string::npos) {
                token = types.substr(0, pos);
                typeVec.push_back(token);
                types.erase(0, pos + delimiterComma.length());
            }
            typeVec.push_back(types);
            
            pos = 0;
            addressStrs = addressStrs.substr(1, addressStrs.length()-2);
            while ((pos = addressStrs.find(delimiterComma)) != std::string::npos) {
                token = addressStrs.substr(0, pos);
                addressStrVec.push_back(token);
                addressStrs.erase(0, pos + delimiterComma.length());
            }
            addressStrVec.push_back(addressStrs);
            
            pos = 0;
            nHashTypes = nHashTypes.substr(1, nHashTypes.length()-2);
            while ((pos = nHashTypes.find(delimiterComma)) != std::string::npos) {
                token = nHashTypes.substr(0, pos);
                nHashTypeVec.push_back(token);
                nHashTypes.erase(0, pos + delimiterComma.length());
            }
            nHashTypeVec.push_back(nHashTypes);
            
            int sigOpsIndex = 0;
            for(size_t i = 0; i < labelVec.size(); i++) {
                std::vector<SigOpStruct> arr;
                int numSigOps = atoi(sigOpCountVec[i].c_str());
                for(int j = 0; j < numSigOps; j++) {
                    SigOpStruct op;
                    op.nScriptChunk = atoi(nScriptChunkVec[sigOpsIndex].c_str());
                    op.type = typeVec[sigOpsIndex];
                    op.addressStr = addressStrVec[sigOpsIndex];
                    op.nHashType = (unsigned long) std::stoll(nHashTypeVec[sigOpsIndex]);
                    arr.push_back(op);
                    sigOpsIndex++;
                }
                //std::pair<std::string,std::vector<SigOpStruct>> mypair(labelVec[i],arr);
                //std::pair<std::string,int> mypair(labelVec[i],3);
                map.insert(std::pair<std::string,std::vector<SigOpStruct>>(labelVec[i],arr));
                //sigOps.map.insert(mypair);
                //printf("Debugging\n");
            }
        }
};
...
static TxBuilderOutputsStruct addTxBuilderOutputs(TxBuilder *bsvTxBuilder, std::vector<PaymentOutput> outputs, std::vector<std::string> addressArray, int64_t walletId, std::string path) {
...
int64_t TxBuilder::buildOutputs () {
    ...
    for(size_t i = 0; i < vout.size(); i++) {
        ...
        /*CDataStream ssTx(SER_NETWORK, PROTOCOL_VERSION | 0);
          vout[i].Serialize(ssTx);
          std::string hex_str = HexStr(ssTx);
          printf("build outputs VOUT: %s\n", hex_str.c_str());*/
    }
    return outAmountBn;
}
...
bool TxBuilder::build(bool useAllInputs) {
    ...
    for (
        ...
        /*CDataStream ssTx(SER_NETWORK, PROTOCOL_VERSION | 0);
        changeTxOut.Serialize(ssTx);
          std::string hex_str = HexStr(ssTx);
          printf("changeTxOut: %s\n", hex_str.c_str()); */
...
std::string Sfp::num2binMint(uint32_t vout, int length) {
    char ch[length];
    ...
    return bufferToString(stringToBuffer(std::string(ch)));
}
...
std::string Sfp::createOutput (TokenDataStruct data, COutPoint linkedTxIn) {
    ...
    std::string linkedTxOutpoint = bufferToString(toLittleEndian(stringToBuffer(linkedTxIn.hash.GetHex()))) + num2binMint(linkedTxIn.n, 4);
...
BuildTokenStruct buildTokenTx(Sfp sfp, std::string hex, std::vector<TokenDataStruct> outputs) {
    ...
    /*CDataStream ssTx00(SER_NETWORK, PROTOCOL_VERSION | 0);
    bsvTxBuilder->tx.Serialize(ssTx00);
    std::string hex_str00 = HexStr(ssTx00);
    printf("hex_str00: %s\n", hex_str00.c_str()); */
    
    tokenTx->importInputs(bsvTxBuilder);
    
    /*CDataStream ssTx01(SER_NETWORK, PROTOCOL_VERSION | 0);
    bsvTxBuilder->tx.Serialize(ssTx01);
    std::string hex_str01 = HexStr(ssTx01);
    printf("hex_str01: %s\n", hex_str01.c_str()); */
    ...
    std::string hex_str02 = HexStr(ssTx02);
    printf("hex_str02: %s\n", hex_str02.c_str()); */
...
static void makeRequest(const char *host_, const char *port, const char *method, const char *path, const char *data, char *responseStr)
{
    int i;

    /* first where are we going to send it? */
    int portno = atoi(port)>0?atoi(port):80;
    char *host = strlen(host_)>0?(char *) host_:(char *) "localhost";
    ...
    /* process response */
    printf("Response:\n%s\n",response);
    snprintf(responseStr, strlen(response) + 1, "%s", response);

    free(message);
}

std::string buildActionToJSON(std::string hex, std::vector<TokenDataStruct> outputs) {
    std::string hexStr("{ \"hex\": ");
    std::string sigOpsStr("\"sigOperations\": [ ");
    std::string assetStr("{ \"asset\": ");
    std::string amountStr("\"amount\": ");
    std::string addressStr("\"address\": ");
    std::string authorizerStr("\"authorizer\": ");
    std::string notesStr("\"notes\": ");
    std::string issuerStr("\"issuer\": ");
    std::string stateStr("\"state\": ");
    std::string closeBraceStr(" }");
    
    std::string json = hexStr + std::string("\"") + hex + std::string("\"");
    ...
    for(size_t i = 0; i < outputs.size(); i++) {
        ...
        json = json + assetStr + std::string("\"") + outputs[i].asset + std::string("\",");
        json = json + amountStr + std::to_string(outputs[i].amount) + std::string(",");
        json = json + addressStr + std::string("\"") + outputs[i].address + std::string("\",");
        json = json + authorizerStr + std::string("\"") +  outputs[i].authorizer + std::string("\",");
        json = json + notesStr + std::string("\"") + outputs[i].notes + std::string("\",");
        json = json + issuerStr + std::string("\"") + outputs[i].issuer + std::string("\",");
        json = json + stateStr + std::string("\"") + outputs[i].state + std::string("\"");
        
        json = json + closeBraceStr;
...
std::string authoriseActionToJSON(std::string hex) {
    std::string hexStr("{ \"hex\": ");
    std::string closeBraceStr(" }");
...
std::string hexFromJSON(std::string json) {
    std::string delimiterHex("hex");
    std::string delimiterLabel("label");
    
    std::string hex = json.substr(json.find(delimiterHex) + 7, (json.find(delimiterLabel) - (json.find(delimiterHex) + 7 + 3)));
...
//    printf("json = %s\n", json.c_str());
    ...
    buildAction.hex = hexFromJSON(json);
    //buildAction.sigOperations = tokenData.bsvTxBuilder->sigOperations;
    buildAction.sigOperations.fromJSON(json);
...
static void build(TxBuilder *bsvTxBuilder, std::vector<TokenDataStruct> outputs, std::string path) {
  ...
  if (bsvTxBuilder->tx.vin.size() <= 0) {
    ...
    for(std::map<std::string, std::vector<TokenDataStruct>>::iterator it = outputsByAuthorizer.begin(); it != outputsByAuthorizer.end(); it++) {
        ...
        if(response.sigOperations.map.size() > 0)
            bsvTxBuilder->sigOperations = response.sigOperations; //FIXME?
        ...
        /*for(size_t i = 0; i < bsvTxBuilder->tx.vin.size(); i++) {
            CDataStream ssTx_in(SER_NETWORK, PROTOCOL_VERSION | 0);
            bsvTxBuilder->tx.vin[i].Serialize(ssTx_in);
            std::string hex_in = HexStr(ssTx_in);
            printf("hex_in = %s\n", hex_in.c_str());
        }
        for(size_t i = 0; i < bsvTxBuilder->tx.vout.size(); i++) {
            CDataStream ssTx_out(SER_NETWORK, PROTOCOL_VERSION | 0);
            bsvTxBuilder->tx.vout[i].Serialize(ssTx_out);
            std::string hex_out = HexStr(ssTx_out);
            printf("hex_out = %s\n", hex_out.c_str());
        }
        
        CDataStream ssTx2(SER_NETWORK, PROTOCOL_VERSION | 0);
        bsvTxBuilder->tx.Serialize(ssTx2);
        std::string hex_str2 = HexStr(ssTx2);
        printf("hex_str2: %s\n", hex_str2.c_str()); */
    }
...
void TxBuilder::inputFromScript (uint256 txHashBuf, uint32_t txOutNum, CTxOut txOut, CScript script, uint32_t nSequence) {
  ...
  CTxIn txIn;
    ...
    /*if(vin.size() == 0) {
        vin.push_back(txIn);
    } else if (!isPubKeyHashOut(vin[vin.size() - 1].scriptSig)){
        vin.push_back(txIn);
    } else {
        CTxIn tmp = vin[vin.size() - 1]; //Should be vin[0]
        vin[vin.size() - 1] = txIn;
...
static void initializeAddresses(std::string path) {
    ...
    if( rc != SQLITE_OK ){
      ...
      sql = (char *) "INSERT INTO ADDRESSES (ID,WALLET_ID,ADDRESS,ADDRESS_INDEX) "  \
             "VALUES (1, 4, 'mpgky2LFXVQ2T3FnhCERuffoGuy6vEJ6iD', 0); ";
      rc = sqlite3_exec(db, sql, callback, 0, &zErrMsg);

      sql = (char *) "INSERT INTO ADDRESSES (ID,WALLET_ID,ADDRESS,ADDRESS_INDEX) "  \
             "VALUES (2, 4, 'mnPrtdcEWwEQtxv1fPdCU7M7T7GcPxTBSZ', 1); ";
      rc = sqlite3_exec(db, sql, callback, 0, &zErrMsg);

      sql = (char *) "INSERT INTO ADDRESSES (ID,WALLET_ID,ADDRESS,ADDRESS_INDEX) "  \
                 "VALUES (3, 4, 'msqwW61BFnYGyYq7NZoYJBuPtVJRBt4Suv', 2); ";
          rc = sqlite3_exec(db, sql, callback, 0, &zErrMsg);

          sql = (char *) "INSERT INTO ADDRESSES (ID,WALLET_ID,ADDRESS,ADDRESS_INDEX) "  \
                 "VALUES (4, 4, 'mibKvBdxAzEPBqyvrweargC94cUG6JSxDQ', 3); ";
          rc = sqlite3_exec(db, sql, callback, 0, &zErrMsg);

        sql = (char *) "INSERT INTO ADDRESSES (ID,WALLET_ID,ADDRESS,ADDRESS_INDEX) "  \
               "VALUES (5, 4, 'mrRBNSvjVMuoDm1Ghiuw7gJvz2cFXzKHxp', 4); ";
        rc = sqlite3_exec(db, sql, callback, 0, &zErrMsg);

        sql = (char *) "INSERT INTO ADDRESSES (ID,WALLET_ID,ADDRESS,ADDRESS_INDEX) "  \
               "VALUES (6, 4, 'mrYKAFeA77isM2TYtQS8AkLbJwuQhtyjWb', 5); ";
        rc = sqlite3_exec(db, sql, callback, 0, &zErrMsg);

      sql = (char *) "INSERT INTO ADDRESSES (ID,WALLET_ID,ADDRESS,ADDRESS_INDEX) "  \
               "VALUES (7, 5, 'mnTAxgwj9i8YyeUzAgJ526q4PkDfuJWzbx', 0); ";
      rc = sqlite3_exec(db, sql, callback, 0, &zErrMsg);

      sql = (char *) "INSERT INTO ADDRESSES (ID,WALLET_ID,ADDRESS,ADDRESS_INDEX) "  \
               "VALUES (8, 5, 'n4RwWKJj9NFsWQ4PRuQV7ZzPgrQPBM3gd6', 1); ";
      rc = sqlite3_exec(db, sql, callback, 0, &zErrMsg);

    sql = (char *) "INSERT INTO ADDRESSES (ID,WALLET_ID,ADDRESS,ADDRESS_INDEX) "  \
               "VALUES (9, 5, 'mmbDk9BXnNXEKeRz1x3AiwJceMSt3U5cvu', 2); ";
      rc = sqlite3_exec(db, sql, callback, 0, &zErrMsg);

      sql = (char *) "INSERT INTO ADDRESSES (ID,WALLET_ID,ADDRESS,ADDRESS_INDEX) "  \
               "VALUES (10, 5, 'mo7ruwvd7QFXRRrsTFViiZZz5K7v63MF2D', 3); ";
      rc = sqlite3_exec(db, sql, callback, 0, &zErrMsg);

      sql = (char *) "INSERT INTO ADDRESSES (ID,WALLET_ID,ADDRESS,ADDRESS_INDEX) "  \
               "VALUES (11, 5, 'mnUgVciU9tidKECmaoJSzuVRPuUc3qfb4R', 4); ";
      rc = sqlite3_exec(db, sql, callback, 0, &zErrMsg);

      sql = (char *) "INSERT INTO ADDRESSES (ID,WALLET_ID,ADDRESS,ADDRESS_INDEX) "  \
               "VALUES (12, 5, 'mnzFTSHyzNnMJBSYFHYn27E62EBcFbUaLj', 5); ";
      rc = sqlite3_exec(db, sql, callback, 0, &zErrMsg);
    
    sql = (char *) "INSERT INTO ADDRESSES (ID,WALLET_ID,ADDRESS,ADDRESS_INDEX) "  \
           "VALUES (13, 4, 'msZ4UzaiXEexpxFM4VTHSVzJgaPv57iQbn', 6); ";
    rc = sqlite3_exec(db, sql, callback, 0, &zErrMsg);
    
    sql = (char *) "INSERT INTO ADDRESSES (ID,WALLET_ID,ADDRESS,ADDRESS_INDEX) "  \
           "VALUES (14, 4, 'n4mfAtRpgaqnuS4QLhB4vHciJkJNLYRrt7', 7); ";
    rc = sqlite3_exec(db, sql, callback, 0, &zErrMsg);
    
    sql = (char *) "INSERT INTO ADDRESSES (ID,WALLET_ID,ADDRESS,ADDRESS_INDEX) "  \
           "VALUES (15, 4, 'mkJgfw5iAXymY6qGrzoZGqCQrGjSjQEGZJ', 8); ";
    rc = sqlite3_exec(db, sql, callback, 0, &zErrMsg);
    
    sql = (char *) "INSERT INTO ADDRESSES (ID,WALLET_ID,ADDRESS,ADDRESS_INDEX) "  \
           "VALUES (16, 4, 'mpA6EHqndca1mBG3FeGsmMGDcjYx7kbRE5', 9); ";
    rc = sqlite3_exec(db, sql, callback, 0, &zErrMsg);
    
    sql = (char *) "INSERT INTO ADDRESSES (ID,WALLET_ID,ADDRESS,ADDRESS_INDEX) "  \
           "VALUES (17, 4, 'mib72xBNmuabKSWrMpFAV3aTRRxn53Mquv', 10); ";
    rc = sqlite3_exec(db, sql, callback, 0, &zErrMsg);
    
    sql = (char *) "INSERT INTO ADDRESSES (ID,WALLET_ID,ADDRESS,ADDRESS_INDEX) "  \
           "VALUES (18, 4, 'mwcS2MRJMzsosvETiVBcyvw2twtRYqWtGD', 11); ";
    rc = sqlite3_exec(db, sql, callback, 0, &zErrMsg);
...
static void initializeWallet (std::string path) {
  sqlite3 *db;
    ...
    sql = (char *) "INSERT INTO WALLETS (ID,USER_ID,MNEMONIC,XPUB,LAST_USED_ADDRESS_INDEX,NEXT_ADDRESS_INDEX) "  \
      ...
      sql = (char *) "INSERT INTO WALLETS (ID,USER_ID,MNEMONIC,XPUB,LAST_USED_ADDRESS_INDEX,NEXT_ADDRESS_INDEX) "  \
            "VALUES (3, 6, 'shiver fashion woman faith vote avoid mail tissue satisfy turkey bring floor'," \
      ...
      sql = (char *) "INSERT INTO WALLETS (ID,USER_ID,MNEMONIC,XPUB,LAST_USED_ADDRESS_INDEX,NEXT_ADDRESS_INDEX) "  \
            "VALUES (4, 7, 'ginger settle marine tissue robot crane night number ramp coast roast critic'," \
                   "''," \
                   "-1, 3); ";
      ...
      sql = (char *) "INSERT INTO WALLETS (ID,USER_ID,MNEMONIC,XPUB,LAST_USED_ADDRESS_INDEX,NEXT_ADDRESS_INDEX) "  \
            "VALUES (5, 8, 'tape argue fetch truck cattle quiz wide equal inform rabbit ranch educate'," \
                   "''," \
                   "-1, 1); ";
...
static void initializeRunWallet (std::string path) {
  ...
  //sql = (char *) "INSERT INTO RUN_WALLETS (ID,PRIVKEY,PUBKEY,ADDRESS) "  \
    ...
    sql = (char *) "INSERT INTO RUN_WALLETS (ID,PRIVKEY,PUBKEY,ADDRESS) "  \
          "VALUES (4, 'cNgeHQBoy1qoz3iuzvgMFNKaBT4VpytjU7zoFNfNkcJbfBBPohgF'," \
                 "'03963ff545acb08a58640308989043d91fcc534c9d5cec4179adfe4de517a3cdd9'," \
                 "'n22U6SStMNhr3QGT4k7D9oKUiDypBEmx4X'); ";
 ...
 /* Execute SQL statement */
  ...
  if( rc != SQLITE_OK ){
    ...
    sql = (char *) "INSERT INTO RUN_WALLETS (ID,PRIVKEY,PUBKEY,ADDRESS) "  \
          "VALUES (5, 'cTL4GU4Dae6aBhEagPooUaobC8ju1C4GiyTwxHt9BuBbmQkAhg9T'," \
                 "'028639937a664a7efee7f38a5305aca86c6c1c3206d1b2897cb1372e20239b3fef'," \
                 "'mmjuAZZ8EWFjWsab6Vc8Lsi2FT8cbM5EUc'); ";
...
extern void initializeDeviceWallet(const char *mnemonic_, const char *path_) {
  sqlite3 *db;
    char *zErrMsg = 0;
    int rc;
    char *sql;
...
//#endif
...
#if MAINNET
    /* Create SQL statement */
    sql = (char *) "CREATE TABLE DEVICE_WALLET_MAIN("  \
      "ID INT PRIMARY KEY     NOT NULL," \
        "PRIVKEY          CHAR(500)," \
        "PUBKEY           CHAR(255)," \
        "ADDRESS       CHAR(255));";
    ...
    if( rc != SQLITE_OK ){
      ...
      fprintf(stdout, "DEVICE_WALLET Table created successfully\n");
    }
...
//static void initializeUsers (const char* path) {
  sqlite3 *db;
    char *zErrMsg = 0;
    int rc;
    char *sql;
  ...
  //sql = (char *) "INSERT INTO USERS (ID,ACTIVE_WALLET_ID,PRIMARY_ADDRESS) "  \
    ...
    //sql = (char *) "INSERT INTO USERS (ID,ACTIVE_WALLET_ID,PRIMARY_ADDRESS,RECEIVE_ADDRESS) "  \
          "VALUES (5, 4,'mkyEzrMcY3DobA5vzcZiA79B7ykqDFJr1L','n2LfpyLFkHaevpWDAiKx9jAwhwHnbsn3hL'); ";
    sql = (char *) "INSERT INTO USERS (ID,ACTIVE_WALLET_ID,PRIMARY_ADDRESS,RECEIVE_ADDRESS) "  \
          "VALUES (5, 4,'tb1q7wt0jfrmgw848n33xgvahzzpdhq95s72n7lw7s','tb1q22w8wv66m5nehz85u3mr0wghjjkcag55wtsvfc'); ";
    ...
    if( rc != SQLITE_OK ) {
      ...
      printf("%lu records returned\n", records.size());
        ...
        if(records.size() == 0) {*/
            ...
            sql = (char *) "SELECT * FROM RPC_BUNDLES;";
            ...
            std::string sql_query0 = std::string("INSERT INTO RPC_BUNDLES (ID,TXHASH,VERSION,INCOUNT,OUTCOUNT,LOCKTIME,BLOCKHEIGHT,TIMESTAMP,TYPE, RECEIVE_AMOUNT,MINT_ID,FROM_ADDRESS,SENDER_ADDRESS,FINGERPRINT) VALUES (") + \
            std::to_string(id) + std::string(", '") + std::string(txHash) + std::string("', ") + std::to_string(version) + std::string(", ") + std::to_string(inCount) + std::string(", ") + std::to_string(outCount) + std::string(", ") + std::to_string(lockTime) + std::string(", ") + std::to_string(blockHeight) + std::string(", ") + std::to_string(timestamp) + std::string(", '") + std::string(type) + std::string("', ") + std::to_string(receiveAmount) + std::string(", '") + std::string(mintId) + std::string("', '") + std::string(fromAddress) + std::string("', '") + std::string(senderAddress) + std::string("', ") + std::to_string(fingerPrint) + std::string("); ");
...
extern long long fileServiceLoadRPCGetFingerPrint(unsigned long index, const char *path_) {
    ...
    if( rc != SQLITE_OK ) {
      ...
      printf("%lu records returned\n", records.size());
        ...
        if(records.size() > index) {
            fingerPrint = std::stoll(records[index][13]);
        }
    }
...
extern long long fileServiceLoadRPCGetVersion(unsigned long index, const char *path_) {
    ...
    if( rc != SQLITE_OK ) {
      ...
      printf("%lu records returned\n", records.size());
        ...
        if(records.size() > index) {
            version = std::stoll(records[index][2]);
        }
    }
...
extern long long fileServiceLoadRPCGetInCount(unsigned long index, const char *path_) {
    ...
    if( rc != SQLITE_OK ) {
      ...
      printf("%lu records returned\n", records.size());
        ...
        if(records.size() > index) {
            inCount = std::stoll(records[index][3]);
        }
    }
...
extern long long fileServiceLoadRPCGetOutCount(unsigned long index, const char *path_) {
    ...
    if( rc != SQLITE_OK ) {
      ...
      printf("%lu records returned\n", records.size());
        ...
        if(records.size() > index) {
            outCount = std::stoll(records[index][4]);
        }
    }
...
extern long long fileServiceLoadRPCGetLockTime(unsigned long index, const char *path_) {
    ...
    if( rc != SQLITE_OK ) {
      ...
      printf("%lu records returned\n", records.size());
        ...
        if(records.size() > index) {
            lockTime = std::stoll(records[index][5]);
        }
    }
...
extern long long fileServiceLoadRPCGetTimestamp(unsigned long index, const char *path_) {
    ...
    if( rc != SQLITE_OK ) {
      ...
      printf("%lu records returned\n", records.size());
        ...
        if(records.size() > index) {
            timestamp = std::stoll(records[index][7]);
        }
    }
...
extern long long fileServiceLoadRPCGetOutputAmount(unsigned long index, const char *txHash, const char *path_) {
    ...
    if( rc != SQLITE_OK ) {
      ...
      printf("%lu records returned\n", records.size());
        ...
        if(records.size() > index) {
            outputAmount = std::stoll(records[index][2]);
        }
    }
...
void createSFPUtxos(std::string path) {
  ...
  if( rc != SQLITE_OK ) {
    ...
    for(size_t i = 0; i < records.size(); i++) {
      ...
      Records records0;
      std::string sql_query0 = std::string("SELECT * FROM SFP_UTXOS WHERE TXID = '") + records[i][7] + std::string("' AND VOUT = ") + records[i][8] + std::string(";");
      sql = (char *) sql_query0.c_str();
        printf("sql_query0 = %s\n", sql_query0.c_str());
...
extern bool isTxidUnspentSFPToken (long long walletId, const char *txid_, const char *path_) {
  sqlite3 *db;
    char *zErrMsg = 0;
    int rc;
    char *sql;
  ...
  std::string sql_query = std::string("SELECT * FROM UTXOS WHERE WALLET_ID = ") + std::to_string(walletId) + \
    ...
    //std::string(" AND TXID = '") + txid + std::string("' AND ASSET_ID != 0;");
    ...
    if(records.size() > 0) {
      //res = true;
        if(strcmp(records[0][10].c_str(), "") == 0) {
            ...
            unsigned long long val = (unsigned long long) std::stoll(records[0][11]);
            do {
                Records records0;
...
extern bool fileServiceIsTxidUnspentSFPToken (const char *txid_, const char *fromAddress_, const char *path_) {
  sqlite3 *db;
    ...
    std::string sql_query0 = std::string("SELECT * FROM ADDRESSES WHERE ADDRESS = '") + fromAddress + \
      std::string("';");
    ...
    if (records.size() == 0){
      ...
      if(records.size() > 0) {
        ...
        if( rc != SQLITE_OK ) {
          ...
          std::string sql_query1 = std::string("SELECT * FROM UTXOS WHERE WALLET_ID = ") +
          std::to_string(walletId) + std::string(" AND ASSET_ID = ") + std::to_string(output.asset.id) + (" AND SPENT_TXID = '';");
...
static std::string getSfpAddressByReceiveAddress(const char* address_, const char *path_) {
    std::string path = std::string(path_) + std::string("/tokens.db");
    printf("path: %s\n", path.c_str());
    ...
    std::string sql_query = std::string("SELECT * FROM USERS WHERE RECEIVE_ADDRESS = '") + std::string(address_) + std::string("';");
    ...
    if( rc != SQLITE_OK ) {
      ...
      printf("%lu records returned\n", records.size());
        ...
        if(records.size() > 0) {
            long long walletId = std::stoll(records[0][1]);
            
            Records records0;
            
            std::string sql_query0 = std::string("SELECT * FROM ADDRESSES WHERE WALLET_ID = ") + std::to_string(walletId) + std::string(" ORDER BY ID DESC;");
...
static std::string getRunAddressByReceiveAddress(const char* address_, const char *path_) {
    std::string path = std::string(path_) + std::string("/tokens.db");
    printf("path: %s\n", path.c_str());
    ...
    if( rc != SQLITE_OK ) {
      ...
      printf("%lu records returned\n", records.size());
        ...
        if(records.size() > 0) {
            ...
            std::string sql_query0 = std::string("SELECT * FROM RUN_WALLETS WHERE ID = ") + std::to_string(walletId) + std::string(";");
...
extern void authorizerSaveTransferWOC(const char *txid_, const char *address_, unsigned long long amount_, const char * mintId_, const char * fromAddress_, unsigned long numTxns, const char *jigId_, const char *path_) {
    printf("txid = %s\n", txid_);
    printf("address = %s\n", address_);
    printf("path = %s\n", path_);
    ...
    std::string txid(txid_);
    //std::string address(address_);
    //std::string address = getRunAddressByReceiveAddress(address_, path_);
    std::string address(address_);
    std::string mintId(mintId_);
    std::string fromAddress(fromAddress_);
    std::string jigId(jigId_);
    ...
    if( rc != SQLITE_OK ) {
      ...
      size_t id = records.size() + 1;
          ...
          std::string sql_query0 = std::string("INSERT INTO RUN_TRANSFERS (ID,TXID,TO_ADDRESS,AMOUNT,MINT_ID,FROM_ADDRESS,NUM_TXNS,JIG_ID) VALUES (") + std::to_string(id) + std::string(", '") + txid + std::string("', '") + address + std::string("', ") + std::to_string(amount) + std::string(", '") + mintId + std::string("', '") + fromAddress + std::string("', ") + std::to_string(numTxns) + std::string(", '") + jigId + std::string("'); ");
...
static TransferRecordRun getTransferRun(long long index, std::string path) {
    sqlite3 *db;
      char *zErrMsg = 0;
      int rc;
      char *sql;
    ...
    if( rc != SQLITE_OK ) {
      ...
      printf("%lu records returned\n", records.size());
        ...
        if(records.size() > 0) {
            ...
            rec.address = records[index][2];
            rec.amount = std::stoll(records[index][3]);
            rec.mintId = records[index][4];
            rec.fromAddress = records[index][5];
            rec.numTxns = std::stoll(records[index][6]);
            rec.jigId = records[index][7];
        }
    }
    sqlite3_close(db);
    return rec;
...
//extern void authorizerCreateSerialization(const char *toAddress, const char *fromAddress, const char *txid, long long vout, long long satoshis, const char *script_) {
    ...
    if((startAmount - rec.amount) > 0) {
        ...
        outputRemainder.asset = output.asset;
        outputRemainder.type = output.type;
        outputRemainder.amount = startAmount - rec.amount;
    ...
    TxBuilderOutputsStruct outputsStruct = addTxBuilderOutputs(bsvTxBuilder, paymentOutputs, addressArray, walletId, path);
    ...
    /*CDataStream ssTx03(SER_NETWORK, PROTOCOL_VERSION | 0);
    bsvTxBuilder->tx.Serialize(ssTx03);
    std::string hex_str03 = HexStr(ssTx03);
    printf("hex_str03: %s\n", hex_str03.c_str());*/
...
extern void authorizerGetNumTxnsForTransferRUN(long long *numTxns, const char *path_) {
    ...
    TransferRecordRun rec = getTransferRun(0, path);
...
extern void authorizerGetTransferDataRUN(long long index, char *txnIdHexStr, int txnIdSize, char *addressHexStr, int addressSize, char *mintIdHexStr, int mintIdSize, char *fromAddressHexStr, int fromAddressSize, long long *amount, char *jigIdHexStr, int jigIdSize, const char *path_) {
    ...
    TransferRecordRun rec = getTransferRun(index, path);
...
extern void authorizerGetPrivKeyDevice(char *privkeyHexStr, int privkeySize, const char *path_) {
    ...
    if( rc != SQLITE_OK ) {
      ...
      if(records.size() > 0) {
        ...
        std::string privKey = records[0][1];
        
        snprintf(privkeyHexStr, privkeySize, "%s", privKey.c_str());
      }
    }
    sqlite3_close(db);
}
